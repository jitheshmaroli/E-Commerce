...
const orderHistory = async (req, res) => {
  try {
    ...
    res.render('users/orders/orderHistory', { orders, user, totalPages, currentPage: page , categoryList,formattedDate});
  } catch (error) {
    console.log(error);
...
const orderDetails = async (req,res) => {
  try {
    ...
    const order = await Order.findById({ _id: orderId })
      ...
      console.log(order)
    res.render('users/orders/orderDetails', { order, user, categoryList});
  } catch (error) {
    console.log(error);
    res.status(500).send('Error retrieving order history');
  }
...
const returnProductPage = async (req, res) => {
  try {
    ...
    res.render('users/orders/returnProduct', { order, item, user });
  } catch (error) {
