...
const shoppingHomeView = async (req, res) => {
    try {
      ...
      res.render('users/pages/shoppingHome', {
...
const checkoutView = async (req, res) => {
  try {
    ...
    return res.render('users/pages/checkout', { cart: null, checkoutItems, addresses: address, user });
  } catch (error) {
...
const productSearchView = async (req, res) => {
  try {
    ...
    res.render('users/pages/productSearch', {
      products: productsWithRating,
      wishlistProducts: wishlistProducts,
      user,
      currentPage: page,
  ...
  const productDetailsView = async (req, res) => {
    try {
      ...
      if (!product) {
        return res
          .status(404)
          .render("users/errors/404", { message: "Product not found" });
      }
      ...
      res.render("users/pages/prodcutDetails", {
